...
def replace_leaf(t, find_value, replace_value):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
        return t if label(t) != find_value else tree(replace_value)
    else:
        new_bs = [replace_leaf(b, find_value, replace_value) for b in branches(t)]
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert lower_bound(y) * upper_bound(y) > 0, 'interval spans zero can not be divisor'
...
def multiple_references_explanation():
    return """a formula to compute with intervals using Alyssa's system 
